...
public class AutentificacionService {
    private static final Logger logger = LoggerFactory.getLogger(AutentificacionService.class);
    ...
    public TokenDto autentificar(String username, String password) {
        ...
        try {
            ...
            Usuario usuarioAutentificado = dao.findByUsername(username).orElse(null);
            token = generaToken(usuarioAutentificado);
    ...
    private String generaToken(Usuario usuario) {
        String token = proveedorToken.fabricaToken(
            usuario.getUsername(), 
            usuario.getRoles()
        );
